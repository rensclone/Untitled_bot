...
const { setupOutboxProcessor } = require('./outbox-processor')
moment.locale('id');
...
const connect = async () => {
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
          ...
          const outboxProcessor = setupOutboxProcessor(sock);
        }
      });
        ...
        if (text.toLowerCase() === 'list') {
            ...
            try {
                await sock.sendMessage(sender, {
                    listMessage: {
                        title: "📱 Daftar Aplikasi Premium",
                        description: "Silakan pilih aplikasi yang ingin Anda lihat:",
                        buttonText: "Pilih Aplikasi",
                        footerText: "UNTITLED STORE",
                        sections: [
                            {
                                title: "Daftar Aplikasi",
                                rows: Object.keys(db.apps).map((app, idx) => ({
                                    title: app,
                                    rowId: `app_${idx}`,
                ...
                console.error("Error sending listMessage:", error);
                // Fallback ke pesan teks biasa jika list message gagal
                const appList = Object.keys(db.apps).map((app, index) => `*${index + 1}. ${app}*`).join('\n');
                await sock.sendMessage(sender, {
                    text: `╭─✧ [ 📜 *DAFTAR APLIKASI PREMIUM* ] ✧─╮\n\n${appList}\n-Untuk melihat detail Aplikasi dan harga, silahkan ketik nama produk yang ada pada list di atas.\nJika ingin melakukan pemesanan silahkan contact owner atau admin yang aktif!.\n ╰────✧ [ *HAVE A GOOD DAY* 😇 ] ✧────╯`
                });
            }
        }

        // Handler untuk membalas ketika user memilih aplikasi dari listMessage
        if (m.message.listResponseMessage) {
            try {
                const selectedId = m.message.listResponseMessage.singleSelectReply.selectedRowId;
                const appIndex = parseInt(selectedId.split('_')[1]);
                const appName = Object.keys(db.apps)[appIndex];
                const appDesc = db.apps[appName];
                if (appName && appDesc) {
                    await sock.sendMessage(sender, { text: `📱 *${appName}*\n\n${appDesc}` });
                }
            } catch (error) {
